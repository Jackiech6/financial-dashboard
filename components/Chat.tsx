...
export function Chat({ ticker }: ChatProps) {
  ...
  const handleSend = async () => {
    ...
    // Calculate updated messages synchronously before state update
    const updatedMessagesForAPI = [...messagesRef.current, userMessage]
    ...
    try {
      // Call chat API with updated messages - ensure array is not empty
      if (!updatedMessagesForAPI || updatedMessagesForAPI.length === 0) {
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ticker,
          messages: updatedMessagesForAPI.map((msg) => ({
            role: msg.role,
            content: msg.content,
          })),
        }),
